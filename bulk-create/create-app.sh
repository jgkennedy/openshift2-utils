...
if [ \! -d /var/lib/openshift/${2}-${1} ]; then
  curl -X POST -k -u $1:pass -H "Content-Type: application/json" -d '{ "name": "'$2'", "cartridges": ["diy-0.1"], "domain_id": "'$1'", "application": { "name": "'$2'" } }' https://localhost/broker/rest/applications
